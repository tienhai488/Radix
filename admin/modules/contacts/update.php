...
if(isGet()){
    ...
    if(!empty($body['id'])){
        ...
        if($n==0){
            setFlashData("msg", "Không tìm thấy liên hệ nào!");
            setFlashData("msg_type", "danger");
            redirect("?module=contacts");
        }else{
            $item = firstRaw("select * from contacts where id = $id");
            $item['status']+=1;
...
if (isPost()) {
    ...
    if (empty($arrErr)) {
        ...
        $result = update("contacts", $dataUpdate,"id = $id");
        if ($result) {
