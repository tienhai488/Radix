...
$data = [
    'pageTitle'=>'Nhân bản danh mục dự án'
];
...
if (isGet()) {
    ...
    if(!empty($body['id'])){
        ...
        if($n!=0){
            $cateDetail = firstRaw("select * from portfolios where id = $id");
            $id = $cateDetail["id"];
            update('portfolios',["duplicate"=>$cateDetail['duplicate'] + 1],"id = $id");
            ...
            insert('portfolios',$cateDetail);
        }else {
            setFlashData('msg',"Không tồn tại danh mục dự án!");
