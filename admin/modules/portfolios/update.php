...
$data = [
    "pageTitle" => "Cập nhập danh mục dự án",
];
...
if(isGet()){
    ...
    if(!empty($body['id'])){
        ...
        if($n==0){
            setFlashData("msg", "Không tìm thấy danh mục dự án!");
            setFlashData("msg_type", "danger");
            redirect("?module=portfolios");
        }else{
            $item = firstRaw("select * from portfolios where id = $id");

            $listImage = getRaw("select image from portfolio_images where portfolio_id = $id");
         

            if(!empty($listImage)){
                foreach ($listImage as $value) {
                    $item['gallery'][] = $value['image'];
                }
            }
...
if (isPost()) {
    ...
    if (empty($arrErr)) {
        ...
        $result = update("portfolios", $dataUpdate,"id = $id");
        if ($result) {
...
<div style="max-width: 1200px;margin:0 auto; ">
    ...
    <form method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" placeholder="Tên danh mục dự án..." class="form-control name-service"
        ...
        <div class="form-group">
            ...
            <input type="text" name="video" placeholder="Link video..." class="form-control"
                value="<?php getValueInput($data, 'video') ?>">
        ...
        <div class="form-group">
            ...
            <select name="portfolio_category_id" class="form-control">
                ...
                foreach ($cates as $item) {
                    ?>
                <option
                    <?php echo !empty($data['portfolio_category_id']) && $data['portfolio_category_id'] == $item['id'] ? "selected" : False ?>
        ...
        <div class="form-group">
            ...
            <div class="gallery-group">
                ...
                <div class="gallery-item mb-2">
                    <div class="row ckfinder-group">
                        <div class="col-9">
                            <input type="text" name="gallery[]" placeholder="Hình ảnh..."
                                class="form-control image-render" value="<?php echo $item ?>">
                        ...
                        <div class="col-1">
                            <button type="button" onclick="return comfirm('Bạn chắc chắn muốn xóa?')"
                                class="btn btn-danger btn-block delete-image"><i class="fa fa-trash"></i></button>
                        </div>
                    </div>
                </div>
                <?php
        ...
        <button type="submit" class="btn btn-primary">Cập nhập danh mục dự án</button>
