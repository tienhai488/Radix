<?php
if (!defined("_INCODE")) {
    die("Access deneil!");
}
$data = [
    "pageTitle" => "Thiết lập dịch vụ",
];
layout("header", "admin", $data);
layout("sidebar", "admin", $data);
layout("breadcrumb", "admin", $data);

if (isGet()) {
    $data = getValueOptions("general_fact");
    echo $data;
    if (empty($data)) {
        $data = [];
    } else {
        $data = json_decode($data,true);
    }
    setFlashData("data", $data);
    echo "<pre>";
    print_r($data);
    echo "</pre>";
}

if (isPost()) {
    $body = getBody();

   

    echo "<pre>";
    print_r($body);
    echo "</pre>";

    setFlashData("data", $body);

    $data = json_encode($body);
    echo $data;

    $dataUpdate = ['opt_value'=>$data];

    $result = update("options", $dataUpdate ,'opt_key = "general_fact"');
    if ($result) {
        setFlashData("msg", "Thiết lập thành tích thành công!");
        setFlashData("msg_type", "success");
    } else {
        setFlashData("msg", "Thiết lập thành tích không thành công!");
        setFlashData("msg_type", "danger");
    }
}

$msg = getFlashData("msg");
$msg_type = getFlashData("msg_type");
$errs = getFlashData("errs");
$data = getFlashData("data");
?>
<div style="max-width: 1200px;margin:0 auto; ">
    <?php getMsg($msg, $msg_type); ?>
    <form action="" method="POST">
        <div class="row">
            <div class="col-6">
                <div class="form-group">
                    <label for="">Tiêu đề chính</label>
                    <input type="text" name="heading" placeholder="Tiêu đề chính..." class="form-control" value="<?php getValueInput(
                        $data,
                        "heading"
                    ); ?>">
                    <?php getMsgErr($errs, "heading"); ?>
                </div>
            </div>
            <div class="col-6">
                <div class="form-group">
                    <label for="">Tiêu đề phụ</label>
                    <input type="text" name="sub_title" placeholder="Tiêu đề phụ..." class="form-control" value="<?php getValueInput(
                        $data,
                        "sub_title"
                    ); ?>">
                    <?php getMsgErr($errs, "sub_title"); ?>
                </div>
            </div>
        </div>

        <div class="form-group">
            <label for="">Mô tả</label>
            <textarea class="editor" class="form-control" placeholder="Mô tả..." name="description"><?php getValueInput(
                            $data,
                            "description"
                        ); ?></textarea>
            <?php getMsgErr($errs, "description"); ?>
        </div>

        <div class="row">
            <div class="col-6">
                <div class="form-group">
                    <label for="">Tên nút bấm</label>
                    <input type="text" name="btn_name" placeholder="Tên nút bấm..." class="form-control" value="<?php getValueInput(
                        $data,
                        "btn_name"
                    ); ?>">
                    <?php getMsgErr($errs, "btn_name"); ?>
                </div>
            </div>
            <div class="col-6">
                <div class="form-group">
                    <label for="">Link nút bấm</label>
                    <input type="text" name="btn_link" placeholder="Link nút bấm..." class="form-control" value="<?php getValueInput(
                        $data,
                        "btn_link"
                    ); ?>">
                    <?php getMsgErr($errs, "btn_link"); ?>
                </div>
            </div>
        </div>

        <hr>

        <button type="submit" class="btn btn-primary">Thiết lập thành tích</button>
        <p><a href="<?php echo _WEB_HOST_ROOT ?>">Quay lại</a></p>

    </form>
</div>

<?php layout("footer", "admin", $data);

?>